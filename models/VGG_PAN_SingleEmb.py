...
class SingleModel(nn.Module):
    def __init__(self, inChannel, embDim, nHeads, nLayers, dropout, nCls):
        ...
        self.backbone=models.vgg16(pretrained=True) # taking pretrained model
        ...
        self.obj_embedder=nn.Sequential(nn.Linear(num_ftrs, embDim)) # from 25088 -> specified object embedding
        self.embDim=embDim

        print("Using Self-Attention: nHeads =", nHeads) # no of heads we'll be using in mutli-head attention
        self.obj_mha = MultiHeadAttention(nHeads,embDim,embDim,embDim,dropout) # applying self-attention across views
    ...
    def forward(self,imgBatch): # imgBatch is shape of the input multi-view image
        ...
        imgFtrs = self.backbone(imgBatch) # embedding output [b*v, 25088], 25088 is the output feature vector dim by VGG16
        # from the embedding of imgBatch
        # generating single-view object embedding
        objEmbs = self.obj_embedder(imgFtrs) # [b*v, emb_dim]
        objEmbs = rearrange(objEmbs, '(b v) e -> b v e', b=b) # rearraning dimensions [b,v, emb_dim]
        
        # taking context and attn for object embedding
        objContext, objAttn = self.obj_mha(objEmbs,objEmbs, objEmbs) # inputting key, query, value vector
        ...
        MVOBJEmbs=F.normalize(torch.mean(objContext, 1), dim=1, p=2) # multi-view object embeddings across all views
