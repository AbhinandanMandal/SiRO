...
class MultiHeadAttention(nn.Module):
    def __init__(self, n_head, d_model, d_k, d_v, dropout=0.1):
        ...
        # initializing weights of linear layer using xavier (glorot) normal initialization
    ...
    def forward(self, q, k, v, mask=None):
        ...
        q = rearrange(self.w_qs(q), 'b l (head k) -> head b l k',
                      head=self.n_head)
        k = rearrange(self.w_ks(k), 'b t (head k) -> head b t k',
                      head=self.n_head)
        v = rearrange(self.w_vs(v), 'b t (head v) -> head b t v',
                      head=self.n_head)
        ...
        # applying attn weights to v
        output = torch.einsum('hblt,hbtv->hblv', [attn, v])
        output = rearrange(output, 'head b l v -> b l (head v)')
        output = self.dropout(self.fc(output))
        output = self.layer_norm(output + residual)
        return output, attn  # returning context and attention
