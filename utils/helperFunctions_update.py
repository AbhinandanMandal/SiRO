...
# plotting informative exampler %
def plot_infoex(infoEx, path):
    fig, ax1 = plt.subplots()
    if len(infoEx) > 0:  # similarly of plot_distance()
        color = 'tab:red'
        ax1.set_xlabel('Epochs')
...
def plot_distance(infoEx, max_intra, min_inter, ratio, path):
    fig, ax1 = plt.subplots()
    # if len(infoEx) > 1:
    # if early convergence happens at epoch 0 or 1
    # or training stops early
    # then no plot gonna save
    # by doing >0 it possibly solve the bottleneck
    if len(infoEx) > 0:
        color = 'tab:red'
        ax1.set_xlabel('Epochs')
