...
"""
Custom training dataset for different datasets and other functions
4 datasets are primarily utilised for this 
1. ObjectPI
2. ModelNet40
3. FG3D
4. OWSC
...
"""
Custom training dataset for ObjectPI (OOWL) dataset
"""
...
"""
Custom training dataset for ModelNet40 (MNet40) dataset
"""
...
"""
 Custom training dataset for FG3D (fine-grained 3D) dataset
"""
...
"""
Custom training dataset for "Object With State Change" (OWSC) datatset
"""


class OWSCTrainDataset(Dataset):
...
"""
The load_class_data function is basically a manual data loader 
for all images of a single object/class, grouped by viewpoints

Feature	                    loadDataset	              load_class_data
Works with DataLoader	      Yes	                      No
Returns batch	         Multiple views stacked	    List of single-view tensors
Use case	               Training pipeline	    Evaluation / feature extraction
...
def load_class_data(i, dataset, datadir, flag, Config):
    temp = list()
    if flag == 0:
        vp = Config.gal_vp  # training view points
        if dataset == "OWSC":
            vp = Config.gal_vp[i+1]
    elif flag == 1:
        vp = Config.probe_vp  # testing view points
        if dataset == "OWSC":
            vp = Config.probe_vp[i+1]
    elif flag == 2:
        vp = Config.val_vp  # validation view points
    else:
