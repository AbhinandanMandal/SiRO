...
# Single view object and category recognition & retrieval
# for OWSC dataset
...
# Multi view object and category recognition and retrieval
# for Object With State Change (OWSC) dataset
...
def evaluate_SI_performance_single(dataset, Config, trcv_model, nview):
    ...
    # returning single view and multi view class, object recognition, retrieval accuracy in single embedding model
...
def evaluate_SI_performance_dual(dataset, Config, trcv_model, nview):
    ...
    with torch.no_grad():
        for j, (test_data, obj_labels, cls_labels) in enumerate(tqdm(testLoader)):
            ...
            label_mv_OE[j] = j
    """ Compute performance from single-image query """
    SV_C_mAP, SV_C_acc, SV_O_mAP, SV_O_acc = NNC_OWSC_SV(list(ref_OE.values()), list(
        ref_OE.values()), list(test_OE.values()), list(test_OE.values()), Config)
    """ Compute performance from multi-image query """
    MV_C_mAP, MV_C_acc, MV_O_mAP, MV_O_acc = NNC_OWSC_MV(list(ref_mv_OE.values()), list(
        ref_mv_OE.values()), list(test_mv_OE.values()), list(test_mv_OE.values()), Config)
    # returning single view and multi view class, object recognition, retrieval accuracy in single embedding model
    return SV_C_mAP, SV_C_acc, SV_O_mAP, SV_O_acc, MV_C_mAP, MV_C_acc, MV_O_mAP, MV_O_acc
