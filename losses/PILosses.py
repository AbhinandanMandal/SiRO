...
class PILossOBJ(nn.Module): # pose-invariant object loss for object embedding space
    ...
    def forward(self, SV_A,SV_N,MV_A,MV_N,size_average=True):
        ...
        dSV_A_SV_N = expand_pairwise_distances(SV_A, SV_N) # output shape [nA, nN, nB]
        ...
        confuser_A_N = torch.argmin(dSV_A_SV_N.reshape(nA*nN,nB),0) # 0 signifies for each cols (batch sample)
