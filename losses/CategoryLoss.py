...
class LossCAT(nn.Module):
    ...
    def __init__(self, Config, gamma=4.0):
        ...
        self.gamma=gamma # margin for softmax loss (controls the seperation length)
        ...
        self.coarse_cls_criterion = losses.LargeMarginSoftmaxLoss(Config.Ncls,self.embDim,margin=self.gamma) # large margin softmax loss
        self.mine_criterion = miners.BatchEasyHardMiner(pos_strategy='hard',neg_strategy='hard') # hard mining
    ...
    def forward(self,catembA,catembN,catlabel):
        ...
        catembA = catembA.reshape(dim[0]*dim[1],self.embDim) # shape as [batch x views, d]
        # e.g.
