...
author used pose invariant object loss, category loss and large margin softmax loss for 
...
trcv_model = DualModel(inChannel=Config.inpChannel, embDim=Config.embedDim, nHeads=hp.nHeads,
...
"""
In PiRO code (https://github.com/sarkar-rohan/PiRO), 
author used pose invariant object loss, category and large margin softmax loss for 
dual embedding model
"""
...
optimizer = optim.Adam(params=trcv_model.parameters(), lr=Config.LR)
...
for epoch in range(Config.Nepochs):
    ...
    trcv_model.eval()
    SV_C_mAP, SV_C_acc, SV_O_mAP, SV_O_acc = evaluate_SI_performance_dual(
...
SV_C_mAP, SV_C_acc, SV_O_mAP, SV_O_acc = evaluate_SI_performance_dual(
    dataset=dataset, Config=Config, trcv_model=trcv_model, nview=1)
MV_C_mAP, MV_C_acc, MV_O_mAP, MV_O_acc = evaluate_SI_performance_dual(
